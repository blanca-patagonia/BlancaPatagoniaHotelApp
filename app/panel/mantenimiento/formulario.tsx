...
export function FormularioOrden({ unidades }: { unidades: { id: string; nombre: string }[] }) {
  ...
  return (
    <form action={accion} className="grid gap-3 sm:grid-cols-2">
      <input name="titulo" placeholder="Título (ej: pérdida de agua)" required className="rounded-lg border border-stone-300 px-3 py-2 text-sm outline-none focus:border-lago-600 sm:col-span-2" />
      <select name="unidad_id" defaultValue="" className="rounded-lg border border-stone-300 px-3 py-2 text-sm outline-none focus:border-lago-600">
        <option value="">Sin unidad / general</option>
        {unidades.map((u) => (
          <option key={u.id} value={u.id}>{u.nombre}</option>
        ))}
      </select>
      <select name="prioridad" defaultValue="media" className="rounded-lg border border-stone-300 px-3 py-2 text-sm outline-none focus:border-lago-600">
        <option value="baja">Prioridad baja</option>
        <option value="media">Prioridad media</option>
        <option value="alta">Prioridad alta</option>
      </select>
      <textarea name="descripcion" placeholder="Descripción" rows={2} className="rounded-lg border border-stone-300 px-3 py-2 text-sm outline-none focus:border-lago-600 sm:col-span-2" />
