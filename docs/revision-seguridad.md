...
4. **Token opaco en la confirmación pública** (migración 0011): la URL usa un `uuid`
   inadivinable (`reservas.token`) en lugar del código enumerable; consultar por el
   código ya devuelve 404. Elimina la fuga de PII por enumeración.
5. **Expiración de reservas `pendiente`** (migración 0011): función
   `expirar_reservas_pendientes(dias)` que cancela las pendientes sin seña con más de
   N días y libera el inventario. Con test de integración.
...
- **Programar** `expirar_reservas_pendientes` por cron (pg_cron / Edge Function) en producción.
