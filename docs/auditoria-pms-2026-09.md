...
| 11 | Trazabilidad, auditoría, seguridad, recuperación | 🟢 Auditoría y RLS son fuertes, y lo que falla en el servidor **se ve en `/panel/errores`** (P1-6 cerrado, con test-contrato sobre `app/` y `lib/`). 🟡 Queda **restaurar un backup**, que no se cierra desde el código |
...
| ~~**P1-6**~~ | ✅ **cerrado** · De 67 `console.*` quedan **15**, todos en archivos donde `console` es la respuesta correcta: el chequeo de salud (el sink escribe en la misma base que vigila), los tres *error boundaries* y la PWA (son cliente; `lib/registro.ts` es `server-only`), el proveedor de correo «consola» (esa ES su salida) y las caídas transitorias de la fuente de divisas (persistir cada 502 ahogaría las señales reales). El test-contrato recorre `app/` y `lib/` enteras y **exige que cada excepción tenga su motivo escrito** | `tests/observabilidad-del-dinero.test.ts` |
...
| Observabilidad | ✅ | P1-6 cerrado: 67 → 15, y los 15 justificados | Falla de firma no se ve | Medio | — | Migrar `console.*` + test-contrato sobre app/ y lib/ | M | — | Firma rechazada aparece en `/panel/errores` |
